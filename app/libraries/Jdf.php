...
class Jdf {
	...
	public static function jdate($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
	 ...
	 if (isset($timestamp)) $timestamp=strtotime($timestamp); else $timestamp="";
	 $T_sec=0;/* <= رفع خطاي زمان سرور ، با اعداد '+' و '-' بر حسب ثانيه */

	 if($time_zone!='local')date_default_timezone_set(($time_zone=='')?'Asia/Tehran':$time_zone);
	 $ts=($timestamp=='' or $timestamp=='now')?time()+$T_sec:Jdf::tr_num($timestamp)+$T_sec;
	...
	public static function jmktime($h='',$m='',$s='',$jm='',$jd='',$jy='',$is_dst=-1){
	 $h=Jdf::tr_num($h); $m=Jdf::tr_num($m); $s=Jdf::tr_num($s);
	...
	public static function jalali_to_gregorian($j_y,$j_m,$j_d,$mod=''){
		$g_y=Jdf::tr_num($j_y); $j_m=Jdf::tr_num($j_m); $j_d=Jdf::tr_num($j_d);/* <= :اين سطر ، جزء تابع اصلي نيست */
	 $d_4=($j_y+1)%4;
